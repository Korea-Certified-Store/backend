...
class SafeDataParserTest {
    ...
    //허수가 들어있는 테스트용 Safe Excel 파일이 제공되었을 때, 영업중인 가게의 이름과 주소를 잘 반환하는지 테스트
    @Test
    public void getAllSafeStoresTest() {
        //given
        //기존 getAllMobeomStores 메서드에 지정되어있는 path에서 Excel file을 읽어들임

        //when
        List<StoreDataByParser> allSafeStores = SafeDataParser.getAllSafeStores("safe_test.xlsx");

        //then
        assertThat(allSafeStores.size()).isGreaterThan(0); //조회된 가게가 1개 이상 있어야한다.
        allSafeStores.stream().forEach(store -> { //조회된 가게들을 모두 조회하면서
