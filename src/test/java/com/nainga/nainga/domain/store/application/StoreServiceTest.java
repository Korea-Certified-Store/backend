...
class StoreServiceTest {
    ...
    @Test
    @DisabledIfSystemProperty(named = "mysql", matches = "true")    //Github Actions Workflow 파일에서 해당 System property를 false로 넘겨주어 skip!
    void findStoresByLocation() {
        ...
        List<Store> storesByLocation = storeService.findStoresByLocation(location1, location2);

        //then
        assertThat(storesByLocation.size()).isEqualTo(stores.size());
    }
}