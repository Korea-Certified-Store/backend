...
/*
안심 식당 관련 데이터들을 파싱하기 위한 클래스
 */
public class SafeDataParser {
    //안심 식당 엑셀 파일로부터 가게 이름과 주소지만을 파싱해서 모두 가져오는 메서드
    //테스트에서 번거롭게 Mocking 파일을 쓰지 않고 Test 파일 이름을 지정해줄 수 있도록 fileName을 파라미터에 지정
    public static List<StoreDataByParser> getAllSafeStores(String fileName) throws RestApiException {
        ...
        String absolutePath = new File("").getAbsolutePath() + File.separator + "src" + File.separator + "main" + File.separator + "resources" + File.separator + "data" + File.separator + "safe" + File.separator;
//        fileName = "safe_20240105.xlsx";   //데이터를 추출한 데이터셋 Excel 파일명
