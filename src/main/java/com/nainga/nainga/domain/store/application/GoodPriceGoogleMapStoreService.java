...
public class GoodPriceGoogleMapStoreService {
        ...
        for (StoreDataByParser storeDataByParser : allGoodPriceStores) {
            ...
            if (resultByGooglePlaceId.isEmpty()) {  //아직 DB에 존재하지 않는 가게인 경우!
                ...
                try {
                    ...
                    Optional<Certification> goodPrice = certificationRepository.findByName("착한가격업소");   //Certification 테이블에 이미 착한가격업소 데이터가 있는지 조회
                    if (goodPrice.isPresent()) {   //만약 존재하는 경우면, Certification은 새로 만들어줄 필요가 없음
                        Optional<StoreCertification> byStoreIdCertificationId = storeCertificationRepository.findByStoreIdCertificationId(store.getId(), goodPrice.get().getId());
                        if (byStoreIdCertificationId.isPresent()) {   //이미 해당 가게가 착한가격업소로 StoreCertification에 등록되어있는 경우
                            storeRepository.save(store);
                        } else {
                            StoreCertification storeCertification = StoreCertification.builder()
                                    .store(store)
                                    .certification(goodPrice.get())
                                    .build();
                            storeCertificationRepository.save(storeCertification);
                            storeRepository.save(store);
                        }
                    } else {    //만약 존재하지 않는 경우라면, Certification도 새로 만들어줘야 함
                        Certification goodPriceReal = Certification.builder()
                                .name("착한가격업소")
                                .build();

                        StoreCertification storeCertification = StoreCertification.builder()
                                .store(store)
                                .certification(goodPriceReal)
                                .build();

                        certificationRepository.save(goodPriceReal);
                        storeCertificationRepository.save(storeCertification);
                        storeRepository.save(store);
                    }
                ...
                Optional<Certification> goodPrice = certificationRepository.findByName("착한가격업소");   //여기도 마찬가지로 Certification 테이블에 이미 착한가격업소 데이터가 있는지 조회

                if (goodPrice.isPresent()) {   //만약에 존재한다면, Certification은 새로 만들어줄 필요가 없다
                    Optional<StoreCertification> byStoreIdCertificationId = storeCertificationRepository.findByStoreIdCertificationId(store.getId(), goodPrice.get().getId());
    ...
    //이 메서드는 createAllGoodPriceStore() 메서드를 개선하기 위해 구현된 메서드입니다.
    //createAllGoodPriceStore()는 대용량 데이터를 한번에 모두 조회하는 반면에 아래 메서드는 API call 비용을 제한하여 해당 비용까지만 조회합니다.
    ...
    public CreateDividedGoodPriceStoresResponse createDividedGoodPriceStores(String fileName, double dollars, int startIndex) {
        ...
        for (int i=startIndex; i< allGoodPriceStores.size(); ++i) {
            ...
            if (resultByGooglePlaceId.isEmpty()) {  //아직 DB에 존재하지 않는 가게인 경우!
                if (dollars - 0.02 < 0) {   //남은 달러와 비교하여 API 호출 비용이 없으면 그 상태에서 중단
                    CreateDividedGoodPriceStoresResponse createDividedGoodPriceStoresResponse = new CreateDividedGoodPriceStoresResponse();
                    createDividedGoodPriceStoresResponse.setDollars(dollars);
                    createDividedGoodPriceStoresResponse.setNextIndex(i);
        ...
        //만약 착한가격업소 리스트 끝까지 잘 돌았다면,
        CreateDividedGoodPriceStoresResponse createDividedGoodPriceStoresResponse = new CreateDividedGoodPriceStoresResponse();
        createDividedGoodPriceStoresResponse.setDollars(dollars);
        createDividedGoodPriceStoresResponse.setNextIndex(-1);
        return createDividedGoodPriceStoresResponse;
    }
